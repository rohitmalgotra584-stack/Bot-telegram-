...
# ======================== CONFIGURATION ========================
BOT_TOKEN = "8758615187:AAEYEWabar079xPHzNawniWcs9PuNAejh3s"
ADMIN_ID = 7353041224
...
def init_db():
    ...
    try:
        c.execute("ALTER TABLE withdrawals ADD COLUMN payout_method TEXT DEFAULT 'upi'")
    except:
        pass

    try:
        c.execute("ALTER TABLE withdrawals ADD COLUMN payout_details TEXT DEFAULT ''")
    except:
        pass
...
def get_withdraw_method_label(method):
    return "Redeem Code" if str(method).lower() == "redeem_code" else "UPI"

def get_withdraw_target(wd):
    if not wd:
        return ""
    return wd["payout_details"] or wd["upi_id"] or ""

def mask_withdraw_target(method, target):
    target = str(target or "")
    if str(method).lower() == "redeem_code":
...
# ======================== NOTIFICATIONS ========================
def send_public_withdrawal_notification(user_id, amount, payout_target, status, txn_id="", payout_method="upi"):
    try:
        user = get_user(user_id)
        name = user["first_name"] if user else "User"
        masked = mask_withdraw_target(payout_method, payout_target)
        target_label = get_withdraw_method_label(payout_method)
        ...
        if status == "approved":
            text = (
                ...
                f"│ 🏦 <b>{target_label}</b>     →  <code>{masked}</code>\n"
...
def show_withdraw(chat_id, user_id):
    ...
    if user["upi_id"]:
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(
            types.InlineKeyboardButton(f"✅ Use Saved UPI: {user['upi_id']}", callback_data="use_saved_upi"),
            types.InlineKeyboardButton("🎟 Withdraw via Redeem Code", callback_data="use_redeem_code_withdraw"),
        ...
        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(
            types.InlineKeyboardButton("✏️ Enter UPI ID", callback_data="enter_new_upi"),
            types.InlineKeyboardButton("🎟 Withdraw via Redeem Code", callback_data="use_redeem_code_withdraw")
        )
        safe_send(
            chat_id,
            f"{pe('pencil')} <b>Choose Withdrawal Method</b>\n\n"
            f"{pe('calendar')} <b>Daily Limit:</b> {daily_limit} withdrawals per day\n"
            f"{pe('calendar')} <b>Today's Withdrawals:</b> {today_withdraws}/{daily_limit}\n\n"
            f"{pe('info')} You can withdraw using:\n"
            f"  • <b>UPI ID</b>\n"
            f"  • <b>Redeem Code</b>\n\n"
            f"{pe('warning')} Choose one option below.",
            reply_markup=markup
        )
...
@bot.callback_query_handler(func=lambda call: call.data == "use_redeem_code_withdraw")
def use_redeem_code_withdraw(call):
    user_id = call.from_user.id
    user = get_user(user_id)
    if not user:
        safe_answer(call, "Error!", True)
        return
    safe_answer(call)
    set_state(user_id, "enter_withdraw_redeem_code")
    min_w = get_setting("min_withdraw")
    max_w = get_setting("max_withdraw_per_day")
    safe_send(
        call.message.chat.id,
        f"{pe('key')} <b>Enter Redeem Code</b>\n\n"
        f"{pe('fly_money')} Balance: ₹{user['balance']:.2f}\n"
        f"{pe('down_arrow')} Min: ₹{min_w} | Max: ₹{max_w}\n\n"
        f"{pe('info')} Example: <code>AMAZON500</code>\n"
        f"{pe('pencil')} Type the redeem code you want to receive:"
    )
...
def universal_handler(message):
    ...
    # --- Enter Withdraw Redeem Code ---
    if state == "enter_withdraw_redeem_code":
        if len(text) < 4:
            safe_send(message.chat.id, f"{pe('cross')} <b>Invalid redeem code!</b>\nPlease enter at least 4 characters.")
            return
        state_data = get_state_data(user_id)
        clear_state(user_id)
        set_state(user_id, "enter_amount", {"withdraw_method": "redeem_code", "payout_details": text})
        user = get_user(user_id)
        min_w = get_setting("min_withdraw")
        max_w = get_setting("max_withdraw_per_day")
        safe_send(
            message.chat.id,
            f"{pe('check')} <b>Redeem Code Saved!</b> <code>{text}</code>\n\n"
            f"{pe('money')} Balance: ₹{user['balance']:.2f}\n"
            f"{pe('down_arrow')} Min: ₹{min_w} | Max: ₹{max_w}\n\n"
            f"{pe('pencil')} Enter withdrawal amount:"
        )
    ...
    if state == "enter_amount":
        ...
        if amount > user["balance"]:
            ...
            return
        state_data = get_state_data(user_id)
        withdraw_method = state_data.get("withdraw_method", "upi")
        payout_details = state_data.get("payout_details") or state_data.get("upi_id") or user["upi_id"]
        method_label = get_withdraw_method_label(withdraw_method)
        icon = pe('key') if withdraw_method == 'redeem_code' else pe('link')
        clear_state(user_id)
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton("✅ Confirm", callback_data=f"cwith|{amount}|{withdraw_method}|{payout_details}"),
        ...
        safe_send(
            ...
            f"{icon} <b>{method_label}:</b> <code>{payout_details}</code>\n\n"
...
def confirm_withdraw_cb(call):
    ...
    try:
        ...
        if len(parts) == 4:
            _, amount_str, withdraw_method, payout_details = parts
        else:
            _, amount_str, payout_details = call.data.split("|", 2)
            withdraw_method = "upi"
    ...
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    w_id = db_lastrowid(
        "INSERT INTO withdrawals (user_id, amount, upi_id, payout_method, payout_details, status, created_at) VALUES (?,?,?,?,?,?,?)",
        (user_id, amount, payout_details, withdraw_method, payout_details, "pending", now)
    )
    ...
    safe_edit(
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"{pe('fly_money')} <b>Amount:</b> ₹{amount}\n"
        f"{pe('info')} <b>Method:</b> {get_withdraw_method_label(withdraw_method)}\n"
        f"{(pe('key') if withdraw_method == 'redeem_code' else pe('link'))} <b>Details:</b> <code>{payout_details}</code>\n"
...
def admin_approve(call):
    ...
    try:
        ...
        pass
    try:
        withdraw_method = wd["payout_method"] if "payout_method" in wd.keys() else "upi"
        payout_details = get_withdraw_target(wd)
        safe_send(
            uid,
        ...
        pass
    send_public_withdrawal_notification(uid, amt, get_withdraw_target(wd), "approved", txn, wd["payout_method"] if "payout_method" in wd.keys() else "upi")
